...
ros::NodeHandle_<ArduinoHardware, 0, 3, 100, 100> nh;
...
long default_pressure_sea_level = 101870;
...
volatile float distance_tmp = 0.00; // use volatile for shared variables
...
void setup() {
  ...
  circumference = 2 * 3.141516 * radius;
    ...
    if (reedVal) {
      ...
      if (currentDebounceTicks == 0) {
          ...
          if(velocity_tmp > max_velocity){
            ...
            // update distance only when valid data is available
            distance_tmp = distance_tmp + circumference;
          }
